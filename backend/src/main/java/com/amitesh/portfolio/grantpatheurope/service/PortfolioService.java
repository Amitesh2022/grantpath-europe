package com.amitesh.portfolio.grantpatheurope.service;

import com.amitesh.portfolio.grantpatheurope.api.CreateRecordRequest;
import com.amitesh.portfolio.grantpatheurope.domain.PortfolioRecord;
...
public class PortfolioService {
    private final CopyOnWriteArrayList<PortfolioRecord> records = new CopyOnWriteArrayList<>(List.of(
            new PortfolioRecord(UUID.randomUUID(), "Eligibility review", "IN_REVIEW", "Funding task", Instant.now()),
            new PortfolioRecord(UUID.randomUUID(), "Partner evidence request", "READY", "Funding task", Instant.now()),
            new PortfolioRecord(UUID.randomUUID(), "Budget approval", "OPEN", "Funding task", Instant.now())
    ));
